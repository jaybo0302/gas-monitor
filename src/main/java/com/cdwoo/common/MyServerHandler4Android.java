...
public class MyServerHandler4Android extends IoHandlerAdapter {
    @Override
...
//        			+msg.split(" ")[27]+ "')");
        	stmt.execute("replace into `gas-data` (id, state, gas, dataUnit, gasLevel, low, high, gas1, gasLevel1, low1, high1, dataUnit1,"
        			+ "temperature, deO2Voltage, deComVoltage, dePowerVoltage, deTemp, `check`, warningState, O2Warning, CH4Warning,createTime) values("
        			+gd.getLocalAddr()+",'"+gd.getState()+"','"+gd.getGas()+"','"+gd.getDataUnit()+"','"+gd.getGasLevel()+"','"+gd.getLow()+"','"+gd.getHigh()+"','"
        	...
        	if (gd.getWarningState() == 1 || gd.getO2Warning() ==1 || gd.getCH4Warning() ==1) {
        		stmt.execute("replace into `gas-warn-data` (deviceId, state, gas, dataUnit, gasLevel, low, high, gas1, gasLevel1, low1, high1, dataUnit1,"
            			+ "temperature, deO2Voltage, deComVoltage, dePowerVoltage, deTemp, `check`, warningState, O2Warning, CH4Warning,createTime) values("
            			+gd.getLocalAddr()+",'"+gd.getState()+"','"+gd.getGas()+"','"+gd.getDataUnit()+"','"+gd.getGasLevel()+"','"+gd.getLow()+"','"+gd.getHigh()+"','"
            			+gd.getGas1()+"','"+gd.getGasLevel1()+"','"+gd.getLow1()+"','"+gd.getHigh1()+"','"+gd.getDataUnit1()+"','"+gd.getTemperature()+"','"+gd.getDeO2Voltage()+"','"
            			+gd.getDeComVoltage()+"','"+gd.getDePowerVoltage()+"','"+gd.getDeTemp()+"','"+gd.getCheck()+"',"+gd.getWarningState()+","+gd.getO2Warning()+","
